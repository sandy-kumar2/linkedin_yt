...
const SocialOptions = ({post}: {post: IPostDocument}) => {
  ...
  const likeOrDislikeHandler = async() => {
    ...
    {
      setLikes(tempLikes);
      throw new Error('Failed to fect like');
    }
  ...
  return (
    <div>
      <div className='text-sm mx-2 p-2 flex items-center justify-between border-b border-gray-300'>
        {
          (likes && likes.length > 0) && (<p className='text-xm text-gray-500 hover:text-blue-500 hover:underline hover:cursor-pointer'>{likes.length}</p>)
        }
        {
